...
?>
		<acos(arg)<?php echo $tag; ?> <?php comment_class( $this->has_children ? 'parent' : '' ); ?> id="comment-<?php comment_ID(); ?>">
				...
				<footer class="comment-meta">
					...
					<div class="comment-metadata">
						...
						<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID, $args ) ); ?>">
							...
							<?php $my_date = the_date('d/m/Y | ', '<p>', '</p>', FALSE); echo $my_date; ?>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="form-author">' . '<label for="author">' . __( 'Name' ) . ( $req ? ' <span class="required"></span>' : '' ) . '</label> ' .
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
		'email'  => '<p class="form-email"><label for="email">' . __( 'Email (hidden)' ) . ( $req ? ' <span class="required"></span>' : '' ) . '</label> ' .
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="form-comment"><label for="comment">' . _x( 'Your Comment', 'noun' ) . '</label> <textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
